...
static int test_main(int argc, const char *argv[]) {
  if (argc < 2) {
    MNN_PRINT("================================================================"
              "========\n");
    MNN_PRINT("Arguments: model.MNN runLoops runMask forwardType numberThread "
              "inputSize precision\n");
    MNN_PRINT("================================================================"
              "========\n");
    return -1;
  ...
  std::shared_ptr<MNN::Interpreter> net = std::shared_ptr<MNN::Interpreter>(
      MNN::Interpreter::createFromFile(fileName), MNN::Interpreter::destroy);
  if (nullptr == net) {
    return 0;
  }
  ...
  backendConfig.precision =
      static_cast<MNN::BackendConfig::PrecisionMode>(precision);
  ...
  if (inputTensor->getType().code == halide_type_uint ||
  ...
  std::string input_0_name = pwd + "input_0" + ".txt";
  dumpTensor2File(&givenTensor, input_0_name.c_str(), orderFileOs);

  if (saveOutput) {
    MNN::TensorCallBack beforeCallBack =
        [&](const std::vector<MNN::Tensor *> &ntensors,
    ...
    MNN::TensorCallBack callBack = [&](const std::vector<MNN::Tensor *>
                                           &ntensors,
                                       const std::string &opName) {
      for (int i = 0; i < ntensors.size(); ++i) {
        auto ntensor = ntensors[i];
        ...
        if (tensor->dimensions() == 4) {
          MNN_PRINT(
              "Dimensions: 4, W,H,C,B: %d X %d X %d X %d, OP name %s : %d\n",
  ...
  // benchmark. for CPU, op time means calc duration; for others, op time means
  // schedule duration.
  {
    int t = runTime;
    ...
    MNN::TensorCallBackWithInfo beforeCallBack =
        [&](const std::vector<MNN::Tensor *> &ntensors,
          ...
          if (opTimes.find(info->name()) == opTimes.end()) {
            opTimes.insert(std::make_pair(info->name(),
                                          std::make_pair(0.0f, info->flops())));
          }
          return true;
        };
    MNN::TensorCallBackWithInfo afterCallBack =
        [&](const std::vector<MNN::Tensor *> &ntensors,
            const OperatorInfo *info) {
    ...
    if (t > 0) {
      ...
      for (auto &iter : opTimes) {
        allOpsTimes.push_back(std::make_pair(
            iter.second.first, std::make_pair(iter.first, iter.second.second)));
      ...
      for (auto &iter : allOpsTimes) {
        opSum += iter.first;
        MNN_PRINT("%*s \t[%s] run %d average cost %f ms, %.3f %%, FlopsRate: "
                  "%.3f %%\n",
                  50, iter.second.first.c_str(),
                  opTypes[iter.second.first].c_str(), runTime,
                  iter.first / (float)runTime, iter.first / sum * 100.0f,
