...
static bool compareOutput(VARP output, const std::string &directName,
                          const std::string &name, Dimensionformat dataFormat,
                          int order) {
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    MNN_ERROR(
        "Usage: ./ModuleBasic.out ${test.mnn} ${Dir} [runMask] [forwardType] "
        "[runLoops] [numberThread] [precision | memory] [cacheFile]\n");
    return 0;
  }
  ...
  {
    AUTOTIME;
    net.reset(Module::load(inputNames, outputNames, modelName.c_str(), rtmgr,
                           &mConfig));
    if (net == nullptr) {
...
#define LOAD_DATA(TYPE)                                                        \
  ...
  for (int i = 0; i < inputs.size(); ++i) {
    inputs[i] = _Input(mInfo->inputs[i].dim, mInfo->inputs[i].order,
                       mInfo->inputs[i].type);
  }
  // Load inputs
  for (int i = 0; i < inputs.size(); ++i) {
  ...
  for (int repeat = 0; repeat < repeatNumber; ++repeat) {
    ...
    if (checkOutput) {
      for (int i = 0; i < outputNames.size(); ++i) {
        ...
        bool success = compareOutput(output, directName, outputNames[i],
                                     mInfo->defaultFormat, i);
        if (!success) {
          modelError = true;
