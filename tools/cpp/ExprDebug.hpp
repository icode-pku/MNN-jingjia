...
static void dumpTensor2File(const MNN::Tensor *tensor, const char *file,
...
static void _initDebug() {
  ...
  MNN::TensorCallBackWithInfo callBack =
      [&](const std::vector<MNN::Tensor *> &ntensors,
          const MNN::OperatorInfo *info) {
        auto opName = info->name();
        if (info->type() == "Copy") {
        ...
        for (int i = 0; i < ntensors.size(); ++i) {
          ...
          if (tensor->dimensions() == 4) {
            ...
            MNN_PRINT("Dimensions: %d, %s, OP name %s : %d\n",
                      tensor->dimensions(), oss.str().c_str(), opName.c_str(),
                      i);
          }
  ...
  MNN::Express::Executor::getGlobalExecutor()->setCallBack(
      std::move(beforeCallBack), std::move(callBack));
}
